...
class DatasetSeries {
    ...
    accept(another: Dataset, anotherSegments: Array<Segment>, threshold: number): boolean {
        ...
        const [ok, _] = calculate(this.segments, anotherSegments, threshold)
...
export default class DatasetStore extends VuexModule {
    ...
    /** 同一シリーズをまとめたデータセット。 */
    private _datasetSeries: Array<Array<Dataset>> = []
     ...
     */
    get series(): Array<Array<Dataset>> {
    ...
    setSearchResult(result: SearchResult) {
        ...
        let series: Array<DatasetSeries> = []
        ...
        result.response.result.results.forEach(r => {
            ...
            if (!accepted) {
                series.push(new DatasetSeries(r, segments))
            }
        })

        this._datasetMap = mapping
        this._searchResult = result
        this._datasetSeries = series.map(s => s.entries)
    }
